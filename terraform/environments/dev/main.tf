...
terraform {
  backend "s3" {
    bucket         = "my-terraform-state2478"         # Bucket used to store Terraform state
    key            = "database-sre/dev/terraform.tfstate" 
    region         = "us-east-2"
    dynamodb_table = "terraform-locks"  # DynamoDB table used for Terraform state locking to prevent concurrent modifications
    profile = "aws1"
  }
}
